import { type DayInfo, type SolarDate, canChiOfYear, getDayInfo, jdFromDate, jdToDate } from "@licham/core";
...
export interface ViecMeta extends ViecDef {
  ...
  tagline: string;
  /** Có hai người cần xem tuổi (cưới hỏi) hay chỉ một. */
  twoPersons: boolean;
}
 ...
 * Điểm "không xung tuổi" tạm cho đủ 25 vì lõi chưa có bảng tuổi xung (xem dayInfo.ts, TODO: Liêm cấp bảng),
 * nên năm sinh hiện chưa ảnh hưởng tới điểm số hay thứ hạng ngày.
 */
export function scoreDay(info: DayInfo, viec: ViecDef): DayScore {
  ...
  const reason = `${isHoangDao ? "Hoàng đạo" : "Hắc đạo"} ${info.thanSatNgay.star}, trực ${info.truc.name}, ${daiKyText}`;

  return { score, scoreLabel, hoangDaoPoints, saoPoints, tuoiXungPoints, daiKyPoints, goodStars, badStars, daiKy, reason };
}
...
export function bestDaysInRange(viec: ViecDef, from: SolarDate, to: SolarDate, limit = 7): DayResult[] {
